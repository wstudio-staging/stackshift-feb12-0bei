...
import { CommonPageData, BlogsData, DefaultSeoData, OtherBlogs } from "types";
...
function Document({
  ...
  return (
    <>
      ...
      {/* Show Blog sections */}
      {data?.blogData && (
        <BlogSections data={data?.blogData} otherBlogs={data?.otherBlogs} />
      )}
    </>
  );
}
...
export const getStaticProps: GetStaticProps = async ({
  ...
  const [page, blogData, globalSEO, allBlogs] = await Promise.all([
    ...
    client
      .fetch(allBlogsQuery)
      .then((results) => {
        //filter out unnecessary blogs
        const currentIndex = results.findIndex(
          (result) => result.slug === params.slug
        );
        const previousBlog = results[currentIndex - 1];
        const nextBlog = results[currentIndex + 1];
